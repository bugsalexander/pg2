use pg2::{
    establish_postgres_pool, establish_redis_pool, query_redis_timeline_1, query_redis_timeline_2,
    query_timeline, NUM_USERS,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(_mode) = args.get(1) {
        ...
        match mode {
            ...
            "redis1" => query_redis_timelines_1(&runtime, &mut rng, &counter),
            "redis2" => query_redis_timelines_2(&runtime, &mut rng, &counter),
...
fn query_redis_timelines_2(runtime: &Runtime, rng: &mut ThreadRng, counter: &Arc<AtomicU32>) {
    let pool = establish_redis_pool();
    ...
    for _i in 0..1_000_000 {
        ...
        runtime.spawn_blocking(move || {
            let mut conn = pool2.get().unwrap();
            query_redis_timeline_2(&mut conn, uid);
            counter_ref.fetch_add(1, Ordering::Relaxed);
        });
    }
}
